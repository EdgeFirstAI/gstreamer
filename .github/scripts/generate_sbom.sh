...
#   2. Defining known system dependencies as CycloneDX components
...
if [ -f "$VERSION_FILE" ]; then
    VERSION=$(grep "version\s*:" "$VERSION_FILE" | head -1 | sed "s/.*'\(.*\)'.*/\1/")
...
# Step 3: Generate dependency SBOM (system dependencies for C/Meson project)
echo "[3/6] Generating dependency SBOM..."

echo "  Defining known system dependencies..."
python3 << EOF
import json
...
# Known system dependencies for this C/Meson project
# These are defined manually since there is no package manager lockfile
components = [
    {
        "type": "library",
        "bom-ref": "gstreamer-1.0",
        "name": "gstreamer",
        "version": "1.20",
        "description": "GStreamer multimedia framework",
        "licenses": [{"license": {"id": "LGPL-2.1-or-later"}}],
        "purl": "pkg:generic/gstreamer@1.20"
    },
    {
        "type": "library",
        "bom-ref": "glib-2.0",
        ...
        "licenses": [{"license": {"id": "LGPL-2.1-or-later"}}],
        "purl": "pkg:generic/glib@2.56"
    },
    {
        "type": "library",
        "bom-ref": "json-glib-1.0",
        "name": "json-glib",
        "version": "1.0",
        "description": "JSON parser for GLib",
        "licenses": [{"license": {"id": "LGPL-2.1-or-later"}}],
        "purl": "pkg:generic/json-glib@1.0"
    },
    {
        "type": "library",
        "bom-ref": "zenoh-c",
        "name": "zenoh-c",
        "version": "1.6",
        "description": "Zenoh C client library",
        "licenses": [{"expression": "Apache-2.0 OR EPL-2.0"}],
        "purl": "pkg:generic/zenoh-c@1.6"
    },
    {
        "type": "library",
        "bom-ref": "edgefirst-schemas",
        "name": "edgefirst-schemas",
        "version": "1.5",
        "description": "EdgeFirst CDR schemas C API",
        "licenses": [{"license": {"id": "Apache-2.0"}}],
        "purl": "pkg:generic/edgefirst-schemas@1.5"
    },
    {
        "type": "library",
    ...
    },
    {
        ...
        "name": "nnstreamer",
        "version": "2.0",
        "description": "NNStreamer neural network inference framework (optional)",
        "licenses": [{"license": {"id": "LGPL-2.1-or-later"}}],
        "purl": "pkg:generic/nnstreamer@2.0"
    }
]
...
print(f"  Defined {len(components)} system dependencies")
EOF
