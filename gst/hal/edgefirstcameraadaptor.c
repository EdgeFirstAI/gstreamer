...
edgefirst_camera_adaptor_start (GstBaseTransform *trans)
{
  ...
  /* Route HAL logs through GST_DEBUG so they're visible with:
   *   GST_DEBUG=edgefirst-hal:5  (DEBUG)
   *   GST_DEBUG=edgefirst-hal:7  (TRACE)
   * The max HAL level is derived from the current GST_DEBUG threshold. */
  {
    GstDebugLevel gst_threshold = gst_debug_category_get_threshold (
        edgefirst_hal_debug);
    hal_log_init_callback (hal_log_to_gst, NULL,
        gst_level_to_hal (gst_threshold));
  }
